...
public class RxFirebaseDatabase {
    ...
    private static final String TAG = RxFirebaseDatabase.class.getSimpleName();

    // huu add new setValue
    @NonNull
    public static Observable<String> setvalue2(@NonNull final DatabaseReference ref, final Object valu) {
        return Observable.create(new ObservableOnSubscribe<String>() {
            @Override
            public void subscribe(final ObservableEmitter<String> emitter) throws Exception {
                ref.setValue(valu).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
